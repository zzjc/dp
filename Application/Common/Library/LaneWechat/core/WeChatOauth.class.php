...
class WeChatOAuth{
    ...
    public static function getCode($redirect_uri, $state=1, $scope='snsapi_base'){
        ...
        $response_type = 'code';
        //构造请求微信接口的URL
       echo  $url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$appid.'&redirect_uri='.$redirect_uri.'&response_type='.$response_type.'&scope='.$scope.'&state='.$state.'#wechat_redirect';
        //file_put_contents('logs/__code'.rand(100,999), $url);
    ...
    public static function getAccessTokenAndOpenId($code){
        ...
        //file_put_contents('__url'.rand(100,999), $url);
        $ret = Curl::callWebServer($url);
        if(isset($ret['openid'])){
            $rUrl = 'http://'.$_SERVER["HTTP_HOST"].'/template/_count_openid_get_times.php?action='.md5('saveTimes').'&openid='.$ret['openid'].'&appid='.WECHAT_APPID;
            Curl::callWebServer($rUrl);
        }
        return $ret;
    }
