...
//require(dirname(__FILE__) . '/AccessToken.class.php');
/**
 * 模板消息接口
 * User: lane
 * Date: 14-10-30
...
class TemplateMessage{
     ...
     *                  'keynote4'=>array('value'=>'欢迎再次购买。', 'color'=>'#173177')
     * );
    ...
    public static function sendTemplateMessage($data, $touser, $templateId, $url, $topcolor='#FF0000'){
        ...
        if( isset($ret['errcode']) && $ret['errcode']!=0 ){
            $ret['zy']['template_id'] = $template2;
            file_put_contents($_SERVER["DOCUMENT_ROOT"].'/log/tpl-err['.date('Y-m-d').']'.time(), var_export($ret,1));
            unset($ret['tplid']);
            unset($ret['xurl']);
        }
        return $ret;
