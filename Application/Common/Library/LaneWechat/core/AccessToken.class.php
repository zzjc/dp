...
class AccessToken{
    ...
    private static function _getAccessToken(){
        ...
        if(!isset($accessToken['access_token'])){
            //return Msg::returnErrMsg(MsgConstant::ERROR_GET_ACCESS_TOKEN, $accessToken['errcode'].':'.$accessToken['errmsg']);
            file_put_contents($_SERVER["DOCUMENT_ROOT"].'/log/access_token['.date('Y-m-d').']'.time(), var_export($accessToken['errcode'].':'.$accessToken['errmsg'].':'.$url,1));
            die;
            //return false;
        }
         ...
         */
        /*echo $f = fopen('access_token.json', 'w+');
       echo  $res=fwrite($f, $accessTokenJson);
        fclose($f);
        die;*/
        file_put_contents($_SERVER["DOCUMENT_ROOT"].'/log/data/access_token.json',$accessTokenJson);
        return $accessToken;
    }
...
//        $accessToken = YourDatabase::get('access_token');
        ...
        $accessTokenJson = file_get_contents($_SERVER["DOCUMENT_ROOT"].'/log/data/access_token.json');
        $accessToken = json_decode($accessTokenJson,true);
    
        //$accessToken['value'] = $data;
         if(!empty($accessToken['access_token'])){
            if(time() < $accessToken['expires_in']+$accessToken['time']-10 ){
                return $accessToken;
            }
        }
        return false;
