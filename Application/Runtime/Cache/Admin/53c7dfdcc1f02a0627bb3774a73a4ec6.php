...
<a href="<?php echo U('topic/add');?>"><button class="layui-btn"><i class="layui-icon">&#xe608;</i> 添加主题</button></a>
</div>
...
<td style="padding-left: 20px;"><?php echo (msubstr(strip_tags($vo["content"]),0,20)); ?></td>
<td align="center"><?php echo (date("Y-m-d",$vo["addtime"])); ?></td>
<td align="center">
<a class="layui-btn layui-btn-mini layui-btn-warm" href="<?php echo U('topic/add',array('id'=>$vo['id'],'act'=>'edit'));?>">修改</a> <a class="layui-btn layui-btn-mini layui-btn-danger del_btn" member-id="<?php echo ($vo["id"]); ?>" title="删除" nickname="<?php echo ($vo["title"]); ?>">删除</a>
</td>
...
layui.use('form',function(){
  ...
  jq('.del_btn').click(function(){
    ...
    layer.confirm('确定删除【'+name+'】?', function(index){
      ...
      jq.post('<?php echo U("topic/dels");?>',{'id':id},function(data){
        if(data.code == 200){
          layer.close(loading);
  ...
  form.on('submit(delete)', function(data){
    ...
    layer.confirm('确定批量删除?', function(index){
      ...
      jq.post('<?php echo U("topic/delss");?>',param,function(data){
        if(data.code == 200){
          layer.close(loading);
          layer.msg(data.msg, {icon: 1, time: 1000}, function(){
            location.reload();
  ...
  form.on('submit(sort)', function(data){
    ...
    layer.confirm('确定调整排序吗?', function(index){
	  ...
	  console.log(data);
      var param = data.field;
      jq.post('<?php echo U("topic/sorts");?>',param,function(data){
        if(data.code == 200){
          layer.close(loading);
          layer.msg(data.msg, {icon: 1, time: 1000}, function(){
            location.reload();
